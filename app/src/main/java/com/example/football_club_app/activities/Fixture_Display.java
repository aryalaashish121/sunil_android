...
public class Fixture_Display extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fixture__display);


        recyclerView = findViewById(R.id.recyclerView);
        LinearLayoutManager layoutManager = new LinearLayoutManager(Fixture_Display.this);
        ...
        Call<List<Fixture>> call = footballapi.getMatchdata();
        call.enqueue(new Callback<List<Fixture>>() {
            @Override
            public void onResponse(Call<List<Fixture>> call, Response<List<Fixture>> response) {
                List<Fixture> playerList = response.body();
                recyclerView.setAdapter(new Fixture_Adapter(getApplicationContext(),playerList));
            }
